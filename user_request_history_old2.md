...
## ExternalContentService 지식베이스 추가 오류 해결 (2025-07-29)

### 문제 해결
1. **지식베이스 추가 실패 오류**: 
   - 오류: 
   - 원인: DocumentStorageService에 외부 콘텐츠 저장용 메서드 부재

2. **해결 방안**:
   - DocumentStorageService에  메서드 추가
   - 외부 콘텐츠(YouTube, 웹사이트)를 파일 없이 텍스트만으로 저장
   - 가상 파일 경로 사용으로 기존 스키마와 호환성 유지

### 추가된 기능
- **외부 콘텐츠 저장**: 파일 업로드 없이 텍스트 콘텐츠 직접 저장
- **메타데이터 관리**: 원본 이름, MIME 타입, 업로드 시간 등 추적
- **자동 요약**: 긴 콘텐츠의 경우 처음 200자로 요약 생성
- **외부 콘텐츠 마킹**:  플래그로 구분

### MCP 서버 상태
- Sequential-thinking 서버는 연결되지만 도구가 로드되지 않는 문제 지속
- 이전에 추가한 디버그 로깅으로 문제 진단 중

### 수정된 파일
- ai-chatbot-mentor/src/services/DocumentStorageService.ts: storeDocument 메서드 추가

------

## MCP sequential-thinking 서버 도구 로딩 문제 디버깅 계속

**요청**: MCP sequential-thinking 서버가 연결되지만 도구가 로드되지 않는 문제를 계속 디버깅

**문제 상황**:
- MCP sequential-thinking 서버는 정상적으로 연결됨
- 직접 테스트 결과 서버가 `sequentialthinking` 도구를 정상 반환함  
- 하지만 MCPClient의 loadTools() 메서드에서 도구가 로드되지 않음

**진행 상황**:
1. MCP 서버 직접 테스트 완료 - 서버와 도구는 정상 작동
2. MCPClient.ts의 loadTools() 메서드에 상세 디버그 로그 추가
3. 응답 구조와 파싱 과정을 세밀하게 로깅하도록 개선

**해결 방향**:
- MCPClient와 실제 서버 간의 통신에서 응답 파싱 문제의심
- 더 자세한 로그를 통해 정확한 원인 파악 중

------

1. AI 답변의 Markdown 컴포넌트를 교체해줘.
2. 사이드바의 메뉴 선택 시 해당 메뉴가 선택된 것으로 보이게 해줘.(현재는 채팅 목록 메뉴만 선택된것으로 나옴)
